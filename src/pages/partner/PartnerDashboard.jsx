...
export default function PartnerDashboard() {
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/v1/partner/operations/dashboard-stats`, {
  ...
  const fetchActiveBooking = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/v1/partner/operations/bookings?status=ACTIVE`, {
        headers: { Authorization: `Bearer ${partnerToken}` }
      });
      if (response.data.success && response.data.data.length > 0) {
        setActiveBooking(response.data.data[0]);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (isOnline) {
      ...
      newSocket.on('connect', () => {
        // Emit connect event so backend puts us in drivers room
        newSocket.emit('driver_connect', {
          ...
          lat: 28.7041, // Mock location for demo (Delhi)
          lng: 77.1025
        });
      });
  ...
  const handleAcceptRide = async () => {
     ...
     setAccepting(true);
     try {
       const response = await axios.post(`${API_URL}/api/v1/partner/operations/bookings/${incomingBooking._id}/accept`, {}, {
         headers: { Authorization: `Bearer ${partnerToken}` }
       });
       if (response.data.success) {
  ...
  const handleUpdateStatus = async (newStatus) => {
     ...
     setUpdating(true);
     try {
       const response = await axios.put(`${API_URL}/api/v1/partner/operations/bookings/${activeBooking._id}/status`, {
          status: newStatus
       }, {
         headers: { Authorization: `Bearer ${partnerToken}` }
       });
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 h-[calc(100vh-80px)] overflow-y-auto relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-3xl shadow-sm border border-gray-100 gap-4">
        ...
        <div className="flex items-center space-x-4 bg-gray-50 p-2 pr-6 rounded-full border border-gray-200">
          <button
            onClick={() => !activeBooking && setIsOnline(!isOnline)}
