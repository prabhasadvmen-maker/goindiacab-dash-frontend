...
export default function UserLayout() {
  ...
  const topMenuItems = [
    ...
    { name: 'Book a Cab', icon: Car, path: '/user/book' },
    { name: 'My Bookings', icon: Calendar, path: '/user/bookings' },
    { name: 'Track Trip', icon: MapPin, path: '/user/track' },
  ...
  const bottomMenuItems = [
    ...
    { name: 'Support', icon: HelpCircle, path: '/user/support' },
        ...
        <div className="h-20 flex items-center px-4 border-b border-slate-800">
          <div className="flex items-center w-full">
            ...
            >
              ...
              <p className="text-xs font-semibold text-[#F59E0B] mt-0.5">User Portal</p>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-6 shadow-sm z-10 shrink-0">
          ...
          <div className="flex items-center space-x-4">
            <button className="p-2 text-gray-500 hover:text-[#fa9600] hover:bg-orange-50 rounded-full transition-colors relative">
            ...
            <div className="group relative">
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-1 border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                <div className="px-4 py-2 border-b border-gray-100">
                  ...
                  <p className="text-xs text-gray-500 truncate">{user?.phone ? `+91 ${user.phone}` : ''}</p>
                </div>
                <button 
