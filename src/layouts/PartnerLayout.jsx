...
export default function PartnerLayout() {
  ...
  const { partnerUser: user, partnerLogout: logout } = useAuthStore();
  ...
  const topMenuItems = [
    { name: 'Overview', icon: LayoutDashboard, path: '/partner/dashboard' },
    { name: 'Bookings', icon: CalendarCheck, path: '/partner/bookings' },
    { name: 'Trips', icon: Navigation, path: '/partner/trips' },
    { name: 'Earnings & Wallet', icon: Wallet, path: '/partner/earnings' },
    { name: 'Vehicle & Documents', icon: Car, path: '/partner/vehicle' },
    { name: 'Notifications', icon: Bell, path: '/partner/notifications' },
  ];

  const bottomMenuItems = [
    { name: 'Support', icon: Headset, path: '/partner/support' },
    { name: 'Profile', icon: User, path: '/partner/profile' },
  ...
  const SidebarLink = ({ item }) => (
    <NavLink
      ...
      className={({ isActive }) => cn(
        ...
        isActive 
          ? "bg-orange-100 text-[#fa9600] font-bold" 
          : "text-gray-600 hover:bg-orange-50 hover:text-[#fa9600]"
      )}
  ...
  return (
    <div className="flex h-screen bg-[#f7f9fc] overflow-hidden">
      ...
      <aside 
        className={cn(
          "bg-white border-r border-gray-200 flex flex-col z-20 transition-all duration-300 ease-in-out shadow-sm",
      ...
      >
        ...
        <div className="h-20 flex items-center px-4 border-b border-gray-100">
          <div className="flex items-center w-full">
            ...
            >
              ...
              <p className="text-xs font-semibold text-[#fa9600] mt-0.5">Partner Portal</p>
            </div>
          </div>
        </div>
        ...
        <div className="shrink-0 border-t border-gray-100 py-4 bg-gray-50">
          <nav className="space-y-1">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-6 shadow-sm z-10 shrink-0">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-semibold text-gray-800 hidden sm:block">Partner Dashboard</h2>
          </div>
            ...
            <div className="group relative">
              <button className="flex items-center space-x-3 bg-gray-50 hover:bg-orange-50 p-2 rounded-xl border border-gray-200 transition-colors">
                <div className="text-right hidden md:block">
                  <p className="text-sm font-semibold text-gray-800">{user?.name || 'Partner'}</p>
                  <p className="text-xs text-green-600 font-medium capitalize">{user?.applicationStatus || 'Approved'}</p>
                </div>
                <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center text-[#fa9600] overflow-hidden border border-orange-200">
                  {user?.profilePhoto ? (
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-1 border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                <div className="px-4 py-2 border-b border-gray-100">
                  <p className="text-sm font-medium text-gray-800">{user?.name || 'Partner'}</p>
